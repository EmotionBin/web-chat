...
- 单点登录(只能在一个页面上登录，若在其他地方也进行登录则会挤掉当前登录的用户) **ok**
- 第三方登录(如github、微信、qq等)
- 加入随机验证码，后端生成 base64 图片
- 没登录会重定向到登录页面，并且携带 `redirect_uri` 参数，登录成功后直接跳转到 `redirect_uri` **ok**

**关于后端**

防止 SQL 注入，先验证用户名，根据输入用户名查找数据库，如有能找到响应的用户再进行密码比对  **ok**  
...
根据获取的定位信息查询天气信息，具体可以调用和风天气API[传送门](https://dev.heweather.com/) **ok**  

### 生成唯一的ID

参考花裤衩 `createUniqueString()` **ok**  
...
1. 时间戳格式化  **ok，使用 dayjs 替代**
2. 防抖、节流 **ok**  
3. open 方法，创建 a 标签打开新页面(参考 d2-admin)，不用 `window.open()`，因为可能会被拦截 **ok**  
4. **utils中所有自己写的通用方法都需要加上$，以$开头，以便于区分** **ok**
...
1. 服务端给前端的 token 不能是真正意义上的 token，应该是一个 hash 字符串，这个字符串是真正的 token 的一个映射，服务端会存储一个 token 映射集合 **ok**
2. token 需要自动刷新，对于活跃用户，只要发送请求并且 token 验证通过就刷新 token，延长 token 过期时间，详见我 github 上 demo 的 token 自动刷新 **ok**
3. 单点登录，只允许在一个地方登录，如果在其他地方又进行了登录则会挤掉当前账号 **ok**
4. token 过期时间默认为半小时 **ok，现在为一个小时**
...
1.个别页面有权限要求，进入的时候判断权限，如果没有权限则不让进入，使用 vue-router 的 meta 进行进入页面时的权限判断 **ok**
...
4. 所有图片尽量都进行压缩，缩小体积，优化性能 [熊猫压缩](https://tinypng.com/)

### 关于css

1. 设置通用 css 类名，如类名 flex-center-row 等价于 flex 行布局水平垂直居中 **ok**
...
1. module 语法，mapMutation等 **ok**
2. vuex自动引入，参考 vue-element-admin 或 d2-admin **ok**
3. 数据持久化，参考 d2-admin **ok**

### 路由

1. 增加 404 页面，未匹配到任何路由的时候渲染 **ok**

### 接口返回的信息

1. 服务端只返回 status, data, code 三个字段，操作成功 status 为 1，操作失败 status 为 0，data 为返回的数据， code 为操作失败的时候对应的错误代码，前端会根据 code 的值来显示提示信息
2. code 可参考如下
